...
# -*- coding: utf-8 -*-
"""
evaluate2.py  –  PairClassifier (BCE, ノイズ負例入り) 用 Recall@K 計測
Top-1,5,10,20,50,100,200,500 を表示。チェックポイントは自動ロード。
"""

import os, csv, numpy as np, torch
from train_pair_classifier_stream import PairClassifier, parse_vec  # ← ファイル名が違うなら直す
...
with torch.no_grad():
    for uid, v in samples:
        ...
        batch = post.unsqueeze(0).expand(acc_mat.size(0), -1)     # (N,D)
