...
train.py ― マルチビュー・コントラスト学習（正則化強化版＋デバッグログ）
  • 従来の csv.reader ベースでチャンク読み込み
  • np.fromstring に count=POST_DIM を指定して DeprecationWarning 回避
  • SetEncoder + GNNビュー を MLPプロジェクションヘッドで対比学習
...
class SetEncoder(nn.Module):
    ...
    def forward(self, posts, pad_mask):
        x = self.input_dropout(posts)                            # (B, S, D_in)
...
class DualViewEncoder(nn.Module):
    ...
    def forward(self, posts, pad_mask, neighbor_list):
        ...
        combo = torch.cat([c, n], dim=1)                 # (B, 2*D_enc)
...
def load_data(posts_csv, edges_csv, acc_npy, max_posts, post_dim):
    ...
    try:
        ...
        sys.exit(f"ERROR: account_vectors.npy が読めません: {e}")
    ...
    with open(posts_csv, encoding='utf-8') as f:
        ...
        for row in tqdm(rdr, desc="Loading posts"):
            uid, _, vec_str = row[0], row[1], row[2]
    ...
    with open(edges_csv, encoding='utf-8') as f:
        rdr = csv.reader(f)
        next(rdr)
        for row in tqdm(rdr, desc="Loading edges"):
            src, dst = row[0], row[1]
    ...
    return account_list, padded, masks, neighbors, rw_dict
...
def train_contrastive():
    ...
    ac_list, posts_cpu, mask_cpu, neighbors, rw_dict = load_data(
    ...
    for ep in range(1, EPOCHS+1):
        ...
        for st in range(0, N, ACCOUNT_BATCH_SIZE):
            ...
            with torch.no_grad():
                ...
                sim = (c_n @ z_n.T) / TAU
                pos = sim.diag()
                neg = sim[~torch.eye(sim.size(0), dtype=bool, device=device)].view(sim.size(0), -1)
                print(f"[DEBUG][Ep{ep} B{steps+1}] pos μ={pos.mean():.4f} σ={pos.std():.4f}  "
                      f"neg μ={neg.mean():.4f} σ={neg.std():.4f}")
            ...
            print(f"[DEBUG][Ep{ep} B{steps+1}] grad_norm={tot_norm:.4f}")

            nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()

            # ── デバッグ学習率 ──
            print(f"[DEBUG][Ep{ep} B{steps+1}] lr={optimizer.param_groups[0]['lr']:.2e}")
